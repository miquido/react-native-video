...
class ExoPlayerView(private val context: Context) :
    ...
    private var subtitleLinesRespected = false
    ...
    fun setSubtitleStyle(style: SubtitleStyle) {
        ...
        if (style.foregroundColor != null && style.backgroundColor != null && style.windowColor != null) {
            subtitleLayout.setStyle(
                CaptionStyleCompat(
    ...
    fun setSubtitleLinesRespected(linesRespected: Boolean) {
    ...
    private inner class ComponentListener : Player.Listener {
        ...
        override fun onCues(cueGroup: CueGroup) {
            val cues: List<Cue> = cueGroup.cues
            if (!subtitleLinesRespected) {
                val noLineCues = ArrayList<Cue>()
                for (cue in cues) noLineCues.add(cue.buildUpon().setLine(Cue.DIMEN_UNSET, Cue.LINE_TYPE_NUMBER).build())
